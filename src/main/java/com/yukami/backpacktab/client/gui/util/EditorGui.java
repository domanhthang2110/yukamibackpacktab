...
public final class EditorGui {
    ...
    public void panel(GuiGraphics guiGraphics, int x, int y, int width, int height) {
        guiGraphics.fill(x, y, x + width, y + height, PANEL_BORDER);
        guiGraphics.fill(x + 1, y + 1, x + width - 1, y + height - 1, PANEL_BACKGROUND);
    }

    public void separator(GuiGraphics guiGraphics, int x, int y, int width) {
        guiGraphics.fill(x, y, x + width, y + 1, SEPARATOR);
    }

    public void sectionHeader(GuiGraphics guiGraphics, String title, int x, int y, int width, int color) {
    ...
    public void label(GuiGraphics guiGraphics, String text, int x, int y, int color) {
        guiGraphics.drawString(Minecraft.getInstance().font, text, x, y, color, false);
    }

    public void button(GuiGraphics guiGraphics, String label, int x, int y, int width, int height, double mouseX, double mouseY, Runnable action) {
        button(guiGraphics, label, x, y, width, height, mouseX, mouseY, action, BUTTON, BUTTON_HOVERED);
    }

    public void primaryButton(GuiGraphics guiGraphics, String label, int x, int y, int width, int height, double mouseX, double mouseY, Runnable action) {
    ...
    private void button(
        ...
        label(guiGraphics, label, textX, textY, 0xFFFFFF);
    }
